...
-- Description:	Trae un listado de esfuerzo anual por agente para exportarlo a Excel.
-- =============================================

IF EXISTS(SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[PA_REPORTES_TRAE_LISTADO_DETALLADO_ESFUERZO_EXCEL]') and objectproperty(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[PA_REPORTES_TRAE_LISTADO_DETALLADO_ESFUERZO_EXCEL]
GO

CREATE PROCEDURE [dbo].[PA_REPORTES_TRAE_LISTADO_DETALLADO_ESFUERZO_EXCEL]
AS
BEGIN TRY
	...
	SELECT
		...
		fechaEmisionCertificado												AS fecha,
		cotizacion.cantidad													AS cantidad,
		montoTransferencia													AS monto
	FROM
