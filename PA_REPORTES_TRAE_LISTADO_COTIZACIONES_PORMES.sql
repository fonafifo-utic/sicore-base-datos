...
-- Description:	Trae un listado de Cotizaciones filtrados por mes y año.
-- =============================================

IF EXISTS(SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[PA_REPORTES_TRAE_LISTADO_COTIZACIONES_PORMES]') and objectproperty(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[PA_REPORTES_TRAE_LISTADO_COTIZACIONES_PORMES]
GO

CREATE PROCEDURE [dbo].[PA_REPORTES_TRAE_LISTADO_COTIZACIONES_PORMES] (@pParametros nvarchar(max))
AS
BEGIN TRY
	BEGIN TRAN
		...
		begin
			...
			select
				...
				cotizacion.fechaHora											as fechaHora,
				convert(varchar, cotizacion.fechaHora, 105) + ' ' +
				convert(varchar(5), cotizacion.fechaHora, 108)					as fechaYHora,
				cotizacion.cantidad												as cantidad,
				cotizacion.precioUnitario										as precioUnitario,
				convert(decimal(10,2), cotizacion.montoTotalDolares)			as montoTotalDolares,
				cotizacion.consecutivo											as consecutivo,
				cotizacion.indicadorEstado										as indicadorEstado,
				dbo.FN_GET_NOMBRE_CORTO_FUNCIONARIO(cotizacion.idFuncionario)	as funcionario,
