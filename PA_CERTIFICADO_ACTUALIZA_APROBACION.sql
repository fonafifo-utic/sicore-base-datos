...
-- Description:	Toma un valor JSON y actualiza las observaciones en Certificado para aprobarlo.
-- =============================================

IF EXISTS(SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[PA_CERTIFICADO_ACTUALIZA_APROBACION]') and objectproperty(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[PA_CERTIFICADO_ACTUALIZA_APROBACION]
GO

CREATE PROCEDURE [dbo].[PA_CERTIFICADO_ACTUALIZA_APROBACION] (@pCertificado as nvarchar(max))
AS
BEGIN TRY
	BEGIN TRAN
		...
		exec PA_ENVIAR_CERTIFICADO_PARA_SERFIRMADO @idFuncionario, @idCertificado;
