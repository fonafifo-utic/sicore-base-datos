...
IF EXISTS(SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[PA_INVENTARIO_ACTUALIZA_AUMENTAR]') and objectproperty(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[PA_INVENTARIO_ACTUALIZA_AUMENTAR]
GO

CREATE PROCEDURE [dbo].[PA_INVENTARIO_ACTUALIZA_AUMENTAR] (@pInventario as nvarchar(max))
AS
BEGIN TRY
	BEGIN TRANSACTION
