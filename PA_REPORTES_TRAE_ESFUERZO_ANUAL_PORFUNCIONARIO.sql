...
IF EXISTS(SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[PA_REPORTES_TRAE_ESFUERZO_ANUAL_PORFUNCIONARIO]') and objectproperty(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[PA_REPORTES_TRAE_ESFUERZO_ANUAL_PORFUNCIONARIO]
GO

CREATE PROCEDURE [dbo].[PA_REPORTES_TRAE_ESFUERZO_ANUAL_PORFUNCIONARIO] (@pIdFuncionario AS BIGINT)
AS
BEGIN TRY
	...
	SELECT
		...
		CONVERT(DECIMAL(10,2), SUM(montoTransferencia))						AS monto,
		CONVERT(
			VARCHAR,
			dbo.FN_GET_ULTIMA_VENTA_PORAGENTE(certificado.idFuncionario),
			105) + ' ' +
		CONVERT(
			VARCHAR(5),
			dbo.FN_GET_ULTIMA_VENTA_PORAGENTE(certificado.idFuncionario),
