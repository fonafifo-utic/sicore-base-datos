...
-- Description:	Trae todos los meses que corresponde a las cotizaciones hechas.
-- =============================================

IF EXISTS(SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[PA_REPORTES_TRAE_MESES_COTIZACION]') and objectproperty(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[PA_REPORTES_TRAE_MESES_COTIZACION]
GO

CREATE PROCEDURE [dbo].[PA_REPORTES_TRAE_MESES_COTIZACION]
AS
BEGIN TRY
	BEGIN TRAN
